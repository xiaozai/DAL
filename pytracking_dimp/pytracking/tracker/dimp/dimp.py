...
class DiMP(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        if state[2]*state[3]>=90*90 :
            if state[3]/state[2] >= 2.2 or state[2]/state[3] >= 2.2:
                sz = self.params.image_sample_size_human
        ...
        #print(math.sqrt(search_area), self.img_sample_sz.prod().sqrt())

        # Target size in base scale
        self.base_target_sz = self.target_sz / self.target_scale
        # print(['self.target_scale',self.target_scale, 'self.base_target_sz ',self.base_target_sz ])
        # ['initial size:', 79.0, 290.0]
        # ['self.target_scale', tensor(1.1825), 'self.base_target_sz ', tensor([245.2423,  66.8074])]
    ...
    def localize_advanced(self, scores, sample_scales):
        ...
        if hasattr(self.params, 'max_speed'):
            if max_score1.item() < self.params.target_not_found_threshold:
                self.params.max_speed+=1
            if  max_score1.item() > self.params.reset_maxspeed_threshold:
                self.params.max_speed=1
            target_disp1[target_disp1>self.params.max_speed]=self.params.max_speed;
            target_disp1[target_disp1<-self.params.max_speed]=-self.params.max_speed;
            print([self.params.max_speed,target_disp1])
