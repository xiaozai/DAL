...
class NetWithBackbone(NetWrapper):
    ...
    def preprocess_image(self, im: torch.Tensor):
        ...
        # torch.Size([13, 4, 288, 288])
        # torch.Size([1, 3, 1, 1])


        if im.shape[1]==6:
            # self._mean = torch.Tensor([0.485, 0.456, 0.406, 22.48/255.0]).view(1, -1, 1, 1)
            # self._std = torch.Tensor([0.229, 0.224, 0.225, 19.60/255.0]).view(1, -1, 1, 1)
            self._mean=torch.Tensor([0.485, 0.456, 0.406,0,0,0]).view(1, -1, 1, 1)
            self._std = torch.Tensor([0.229, 0.224, 0.225,1,1,1]).view(1, -1, 1, 1)
