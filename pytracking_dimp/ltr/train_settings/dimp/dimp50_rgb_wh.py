...
from ltr.dataset import Lasot, Got10k, TrackingNet, MSCOCOSeq,PrincetonRGBD
from ltr.data import processing, sampler, LTRLoader
from ltr.models.tracking import dimpnet, dimpnet_wh
...
def run(settings):
    ...
    dataset_val = sampler.DiMPSampler([ptb_train], [1], samples_per_epoch=5000, max_gap=30,
                                      num_test_frames=3, num_train_frames=3,
                                      processing=data_processing_val)
    ...
    net = dimpnet_wh.dimpnet50(filter_size=settings.target_filter_sz, backbone_pretrained=True, optim_iter=5,
    ...
    objective = {'wh': nn.SmoothL1Loss(), 'iou': nn.MSELoss(), 'test_clf': ltr_losses.LBHinge(threshold=settings.hinge_threshold)}

    loss_weight = {'wh': 1, 'iou': 1, 'test_clf': 100, 'test_init_clf': 100, 'test_iter_clf': 400}

    #actor = actors.DiMPActor(net=net, objective=objective, loss_weight=loss_weight)
    actor = actors.DiMPActor_WH(net=net, objective=objective, loss_weight=loss_weight)
    ...
    optimizer = optim.Adam([{'params': actor.net.classifier.filter_initializer.parameters(), 'lr': 5e-5},
                            ...
                            {'params': actor.net.wh_regressor.parameters(), 'lr':2e-4},
