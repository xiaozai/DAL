...
from ltr.dataset import Lasot, Got10k, TrackingNet, MSCOCOSeq,PrincetonRGBD,StcRGBD
from ltr.dataset import kevinlaiRGBD
from ltr.data import processing, sampler, LTRLoader
from ltr.models.tracking import dimpnet, dimpnet_rgbd, dimpnet_rgbd_cls
...
def run(settings):
    ...
    net = dimpnet_rgbd_cls.dimpnet50(filter_size=settings.target_filter_sz, backbone_pretrained=True, optim_iter=5,
    ...
    # Optimizer
    optimizer = optim.Adam([{'params': actor.net.classifier.conv_0.parameters(), 'lr': 0.1*5e-5}, #new layer
                            {'params': actor.net.classifier.filter_initializer.parameters(), 'lr': 0.1*5e-5},
                            {'params': actor.net.classifier.filter_optimizer.parameters(), 'lr': 0.1*5e-4},
                            {'params': actor.net.classifier.feature_extractor.parameters(), 'lr': 0.01*5e-5},
                            {'params': actor.net.classifier.feature_extractor_depth.parameters(), 'lr': 0.1*5e-5},
                            {'params': actor.net.bb_regressor.parameters(), 'lr': 0.1*2e-4},
                            {'params': actor.net.feature_extractor.parameters(), 'lr': 0.01*2e-5},
