...
class DiMPActor_WH(BaseActor):
    ...
    def __call__(self, data):
        ...
        """
        # Run network
        target_scores, wh_pred = self.net(train_imgs=data['train_images'],
        ...
        # print(['train_images', data['train_images'][0].shape])
        # print(['test_images', data['test_images'][0].shape])
        # ['train_images', torch.Size([10, 3, 288, 288])]
        # ['test_images', torch.Size([10, 3, 288, 288])]


        #print(data['train_anno'].shape)torch.Size([3, 10, 4])
        # print(data['test_label'].shape)torch.Size([3, 10, 19, 19])
        # print(data['proposal_iou'].shape)torch.Size([3, 10, 8])
        #print(wh_pred.shape)torch.Size([3, 10, 16])
        #print(data['test_anno'].shape)#torch.Size([3, 10, 4])
        # Compute loss for ATOM IoUNet
        #loss_wh = self.loss_weight['iou'] * self.objective['iou'](iou_pred, data['proposal_iou'])

        #wh_ref =data['train_anno'][0,0,2:4]
        wh_ref=data['train_images'][0].shape[2:4]  #h,w
        wh_test=data['test_anno'][:,:,2:4] #h,w
        wh_test[:,:,0]=torch.log(wh_test[:,:,0]/wh_ref[0]) #h
        wh_test[:,:,1]=torch.log(wh_test[:,:,1]/wh_ref[1]) #w
        wh_pred=torch.log(wh_pred)
        loss_wh = self.loss_weight['wh'] * self.objective['wh'](wh_pred, wh_test)
        #print(['wh_ref',wh_ref,'wh_test',wh_test])
        ...
        # Total loss
        loss = loss_wh + loss_target_classifier + loss_test_init_clf + loss_test_iter_clf

        # Log stats
        stats = {'Loss/total': loss.item(),
                 'Loss/iou': loss_wh.item(),
