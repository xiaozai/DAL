...
class StcRGBD(BaseDataset):
    ...
    def __init__(self, root=None, image_loader=opencv_loader, vid_ids=None, split=None):
        ...
        """
        root = env_settings().stc_dir if root is None else root
        print('init StcRGBD, root is %s'%root)
    ...
    def _build_sequence_list(self, vid_ids=None, split=None):
        if split is not None:
            ...
            if split == 'train':
                file_path = os.path.join(ltr_path, 'data_specs', 'stc_list.txt')
            else:
    ...
    def _read_anno(self, seq_id):
        ...
        anno_file = os.path.join(seq_path, "%s.txt"%('GT'))
    ...
    def _get_frame_path(self, seq_path, frame_id):
        return os.path.join(seq_path, 'RGB/{:08}.png'.format(frame_id+1))    # frames start from 1
