...
class ToGrayscale(Transform):
    ...
    def transform(self, img, do_grayscale):
        if do_grayscale:
            ...
            if img.shape[2]==3:
                img_gray = cv.cvtColor(img.astype(np.float32), cv.COLOR_RGB2GRAY)
                return np.stack([img_gray, img_gray, img_gray], axis=2)
            if img.shape[2]==4:
                img_gray = cv.cvtColor(img[:,:,:3].astype(np.float32), cv.COLOR_RGB2GRAY)
                img_depth= img[:,:,3].astype(np.float32)
                return np.stack([img_gray, img_gray, img_gray, img_depth], axis=2)
