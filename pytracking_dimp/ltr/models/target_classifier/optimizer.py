...
class DiMPSteepestDescentGN(nn.Module):
    ...
    def __init__(self, settings, num_iter=1, feat_stride=16, init_step_length=1.0,
        ...
        self.depthconv=self.settings.depthaware_for_classifer_optimizer
        ...
        if self.depthconv:
            self.label_map_predictor = DepthConv(num_dist_bins, 1, kernel_size=1, padding=0, bias=False)
        else:
        ...
        if self.depthconv == True:
            self.target_mask_predictor = nn.Conv2d(num_dist_bins, 1, kernel_size=1, bias=False)
            if mask_act == 'sigmoid':
                init_bias = 0.0
            elif mask_act == 'linear':
                init_bias = 0.5
            self.target_mask_predictor.weight.data = mask_init_factor * torch.tanh(2.0 - d) + init_bias

        #plan2
        # if self.depthconv:
        #     self.target_mask_predictor = DepthConv(num_dist_bins, 1, kernel_size=1, padding=0, bias=False)
        # else:
        #     self.target_mask_predictor = nn.Conv2d(num_dist_bins, 1, kernel_size=1, bias=False)
        # if mask_act == 'sigmoid':
        #     init_bias = 0.0
        ...
        if self.depthconv:
            self.spatial_weight_predictor = DepthConv(num_dist_bins, 1, kernel_size=1, padding=0, bias=False)
        else:
    ...
    def forward(self, weights, feat, bb, train_depths, test_depths, sample_weight=None, num_iter=None, compute_losses=True):
        ...
        if self.depthconv:
            #print(dist_map.shape, train_depths.shape, test_depths.shape)torch.Size([12, 100, 19, 19]) torch.Size([3, 4, 288, 288]) torch.Size([3, 4, 288, 288])
            train_depths=train_depths.view(train_depths.shape[0]*train_depths.shape[1], train_depths.shape[2], train_depths.shape[3])
            label_map = self.label_map_predictor(dist_map,train_depths).view(num_images, num_sequences, *dist_map.shape[-2:])
            target_mask = self.target_mask_predictor(dist_map,train_depths).view(num_images, num_sequences, *dist_map.shape[-2:])
            if self.mask_act == 'sigmoid':
                target_mask= F.sigmoid(target_mask)
            spatial_weight = self.spatial_weight_predictor(dist_map,train_depths).view(num_images, num_sequences, *dist_map.shape[-2:])
        else:
        ...
        #print(['target_mask', target_mask.shape, target_mask.min(),target_mask.max()])
        #save target_mask, maybe we can use it as a depth agent
