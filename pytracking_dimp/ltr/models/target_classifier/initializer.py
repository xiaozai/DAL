...
class FilterInitializerLinear(nn.Module):
    ...
    def __init__(self, settings, filter_size=1, feature_dim=256, feature_stride=16, pool_square=False, filter_norm=True,
                 conv_ksz=3):
        super().__init__()
        ...
        self.depthconv=self.settings.depthaware_for_classifer_init
        if self.depthconv:
            self.filter_conv = DepthConv(feature_dim, feature_dim, kernel_size=conv_ksz, padding=conv_ksz // 2)
        else:
        ...
        for m in self.filter_conv.modules():
            if isinstance(m, nn.Conv2d):
                n = m.kernel_size[0] * m.kernel_size[1] * m.out_channels
                m.weight.data.normal_(0, math.sqrt(2. / n))
                if m.bias is not None:
    ...
    def forward(self, feat, bb,train_depths, test_depths):
        """Runs the initializer module.
        Note that [] denotes an optional dimension.
        args:
            feat:  Input feature maps. Dims (images_in_sequence, [sequences], feat_dim, H, W).
        ...
        #print(['feat.shape',feat.shape, 'train_depths.shape ', train_depths.shape ])
        ...
        if self.depthconv:
            ...
            feat = self.filter_conv(feat, train_depths)
        else:
...
class FilterInitializerLinear_rgbd(nn.Module):
    """Initializes a target classification filter by applying a linear conv layer and then pooling the target region.
    args:
        filter_size:  Size of the filter.
        feature_dim:  Input feature dimentionality.
    ...
    def __init__(self, filter_size=1, feature_dim=256, feature_stride=16, pool_square=False, filter_norm=True,
        ...
        self.filter_conv_0 = nn.Conv2d(int(2*feature_dim), feature_dim, kernel_size=conv_ksz, padding=conv_ksz // 2)
        self.filter_conv = nn.Conv2d(feature_dim, feature_dim, kernel_size=conv_ksz, padding=conv_ksz // 2)
        self.filter_pool = FilterPool(filter_size=filter_size, feature_stride=feature_stride, pool_square=pool_square)
        self.filter_norm = filter_norm
    ...
    def forward(self, feat, bb):
        ...
        feat = self.filter_conv(self.filter_conv_0(feat.view(-1, feat.shape[-3], feat.shape[-2], feat.shape[-1])))
