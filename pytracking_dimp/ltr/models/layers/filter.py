...
def apply_filter(feat, filter):
    ...
    #print(feat.shape, filter.shape)torch.Size([1, 512, 18, 18]) torch.Size([1, 512, 4, 4])
...
def apply_filter_depthaware(feat, filter, depth, alpha):
    """Applies the filter on the input features (feat).
    args:
        feat: These are the input features. Must have dimensions (images_in_sequence, sequences, feat_dim, H, W) or  (images_in_sequence, feat_dim, H, W)
        filter: The filter to apply. Must have dimensions (output, feat_dim, fH, fW) or (output, sequence, feat_dim, fH, fW)
        depth The depth map to apply. Must have dimensions (images_in_sequence, 1, fH, fW) or (images_in_sequence, sequences, 1, fH, fW)
        alpha: scalar
    output:
    ...
    if multiple_filters:
        ...
        for seq in range(num_sequences):
            ...
            #print(feat_seq.shape, filter_seq.shape, depth_seq.shape)
            #torch.Size([3, 512, 18, 18]) torch.Size([1, 512, 4, 4]) torch.Size([3, 1, 18, 18])
            scores_seq=depth_conv(feat_seq,depth_seq,filter_seq,None,stride=1,padding=padding,dilation=1)
            #print(scores.shape)torch.Size([3, 1, 19, 19])
            scores[:,seq,:, :,:]=scores_seq
        return scores.view(num_images, num_sequences, scores.shape[-2], scores.shape[-1])

    #print(feat.shape, filter.shape, depth.shape)torch.Size([1, 512, 18, 18]) torch.Size([1, 512, 4, 4]) torch.Size([1, 1, 18, 18])
    depth =alpha*depth
