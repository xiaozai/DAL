...
class DiMPnet_rgbd(nn.Module):
    ...
    def forward(self, train_imgs, test_imgs, train_bb, test_proposals, *args, **kwargs):
        ...
        # Get bb_regressor features for depth channel
...
def dimpnet18(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
    ...
    # DiMP network
    net = DiMPnet_rgbd(feature_extractor=backbone_net, classifier=classifier, bb_regressor=bb_regressor,
...
def dimpnet50(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
    ...
    bb_regressor = bbmodels.AtomIoUNet_rgbd(input_dim=(4*128,4*256), pred_input_dim=iou_input_dim, pred_inter_dim=iou_inter_dim)


    # DiMP network
    net = DiMPnet_rgbd(feature_extractor=backbone_net, feature_extractor_depth=backbone_net_depth,classifier=classifier, bb_regressor=bb_regressor,
                  classification_layer=classification_layer, bb_regressor_layer=['layer2', 'layer3'])
    return net
