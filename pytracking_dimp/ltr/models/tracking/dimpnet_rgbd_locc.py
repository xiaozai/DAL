...
class DiMPnet_rgbd_locc(nn.Module):
    """The DiMP network.
    args:
    ...
    def __init__(self, feature_extractor, feature_extractor_depth, classifier, bb_regressor, occ_classifer, classification_layer, bb_regressor_layer, train_feature_extractor=True):
        super().__init__()
    ...
    def forward(self, train_imgs, test_imgs, train_bb, test_proposals, *args, **kwargs):
        ...
        #print([train_bb.shape, test_proposals.shape])[torch.Size([3, 4, 4]), torch.Size([3, 4, 8, 4])]


        occ_state = self.occ_classifer(train_feat_clf, test_feat_clf, train_feat_clf_depth, test_feat_clf_depth, target_scores[-1], iou_pred, train_bb, test_proposals)
...
def dimpnet18(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
    ...
    # DiMP network
    net = DiMPnet_rgbd_locc(feature_extractor=backbone_net, classifier=classifier, bb_regressor=bb_regressor,
...
def dimpnet50(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
    ...
    # Occ clssifier module
    occ_classifer= occ_clf.LogisticFilter()

    # DiMP network
    net = DiMPnet_rgbd_locc(feature_extractor=backbone_net, feature_extractor_depth=backbone_net_depth,classifier=classifier, bb_regressor=bb_regressor, occ_classifer=occ_classifer,
                  classification_layer=classification_layer, bb_regressor_layer=['layer2', 'layer3'])
    return net
