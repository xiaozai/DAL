...
class AtomIoUNet_rgbd_depred(nn.Module):
    ...
    def __init__(self, input_dim=(128,256), pred_input_dim=(256,256), pred_inter_dim=(256,256)):
        ...
        if True:#depth
            self.conv3_1r_d = conv(input_dim[0], 128, kernel_size=3, stride=1)
            self.conv3_1t_d = conv(input_dim[0], 256, kernel_size=3, stride=1)

            self.conv3_2t_d = conv(256, pred_input_dim[0], kernel_size=3, stride=1)

            self.prroi_pool3r_d = PrRoIPool2D(3, 3, 1/8)
            self.prroi_pool3t_d = PrRoIPool2D(5, 5, 1/8)

            self.fc3_1r_d = conv(128, 256, kernel_size=3, stride=1, padding=0)

            self.conv4_1r_d = conv(input_dim[1], 256, kernel_size=3, stride=1)
            self.conv4_1t_d = conv(input_dim[1], 256, kernel_size=3, stride=1)

            self.conv4_2t_d = conv(256, pred_input_dim[1], kernel_size=3, stride=1)

            self.prroi_pool4r_d = PrRoIPool2D(1, 1, 1/16)
            self.prroi_pool4t_d = PrRoIPool2D(3, 3, 1 / 16)

            self.fc34_1024to512=conv(256*4, 256*2, kernel_size=1, stride=1, padding=0)
            self.fc34_3r_d = conv(256 + 256, pred_input_dim[0], kernel_size=1, stride=1, padding=0)
            self.fc34_4r_d = conv(256 + 256, pred_input_dim[1], kernel_size=1, stride=1, padding=0)

            self.fc3_rt_d = LinearBlock(pred_input_dim[0], pred_inter_dim[0], 5)
            self.fc4_rt_d = LinearBlock(pred_input_dim[1], pred_inter_dim[1], 3)

        self.iou_predictor_0=nn.Linear(2*(pred_inter_dim[0]+pred_inter_dim[1]), pred_inter_dim[0]+pred_inter_dim[1], bias=True)
    ...
    def predict_iou(self, modulation, feat, proposals):
        ...
        fc34_3_r, fc34_4_r,fc34_3_r_d, fc34_4_r_d  = modulation
        c3_t, c4_t, c3_t_d, c4_t_d = feat
        ...
        c3_t_att_d = c3_t_d * fc34_3_r_d.view(batch_size, -1, 1, 1)
        c4_t_att_d = c4_t_d * fc34_4_r_d.view(batch_size, -1, 1, 1)
        ...
        roi3t_d = self.prroi_pool3t_d(c3_t_att_d, roi2)
        roi4t_d = self.prroi_pool4t_d(c4_t_att_d, roi2)

        fc3_rt = self.fc3_rt(roi3t)
        fc4_rt = self.fc4_rt(roi4t)
        fc3_rt_d = self.fc3_rt_d(roi3t_d)
        fc4_rt_d = self.fc4_rt_d(roi4t_d)

        fc34_rt_cat = torch.cat((fc3_rt, fc4_rt,fc3_rt_d,fc4_rt_d), dim=1)

        fc34_rt_cat = self.iou_predictor_0(fc34_rt_cat)
    ...
    def get_modulation(self, feat, bb):
        ...
        feat3_r, feat4_r, feat3_r_d, feat4_r_d = feat
        ...
        c3_r_d = self.conv3_1r_d(feat3_r_d)
        roi3r_d = self.prroi_pool3r_d(c3_r_d, roi1)
        fc3_r_d = self.fc3_1r_d(roi3r_d)

        c4_r_d = self.conv4_1r_d(feat4_r_d)
        roi4r_d = self.prroi_pool4r_d(c4_r_d, roi1)

        # Concatenate from block 3 and 4
        fc34_r = torch.cat((fc3_r, roi4r, fc3_r_d, roi4r_d), dim=1)
        fc34_r = self.fc34_1024to512(fc34_r)

        fc34_3_r = self.fc34_3r(fc34_r)
        fc34_4_r = self.fc34_4r(fc34_r)
        fc34_3_r_d = self.fc34_3r_d(fc34_r)
        fc34_4_r_d = self.fc34_4r_d(fc34_r)

        return fc34_3_r, fc34_4_r,fc34_3_r_d,fc34_4_r_d
    ...
    def get_iou_feat(self, feat2):
        ...
        feat3_t, feat4_t, feat3_t_d, feat4_t_d = feat2
        c3_t = self.conv3_2t(self.conv3_1t(feat3_t))
        c4_t = self.conv4_2t(self.conv4_1t(feat4_t))
        c3_t_d = self.conv3_2t_d(self.conv3_1t_d(feat3_t_d))
        c4_t_d = self.conv4_2t_d(self.conv4_1t_d(feat4_t_d))

        return c3_t, c4_t, c3_t_d, c4_t_d
