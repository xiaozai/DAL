...
class WHregressorNet(nn.Module):
    """Network module for widht and height prediction.
    ...
    def __init__(self, input_dim=(128,256), pred_input_dim=(256,256), pred_inter_dim=(256,256)):
        ...
        self.fc3_rt = LinearBlock(pred_input_dim[0], pred_inter_dim[0], 36)#5)
        self.fc4_rt = LinearBlock(pred_input_dim[1], pred_inter_dim[1], 18)#3)

        #self.iou_predictor = nn.Linear(pred_inter_dim[0]+pred_inter_dim[1], 1, bias=True)
        self.wh_predictor = nn.Linear(pred_inter_dim[0]+pred_inter_dim[1], 2, bias=True)
    ...
    def forward(self, feat1, feat2, bb1, proposals2):
        ...
        pred_wh = self.predict_wh(modulation, iou_feat, proposals2)
        return pred_wh.view(num_images, num_sequences, -1)

    def predict_wh(self, modulation, feat, proposals):
        """Predicts wh error for the give proposals.
        args:
        ...
        #print([c3_t.shape, fc34_3_r.shape])[torch.Size([30, 256, 36, 36]), torch.Size([30, 256])]
        ...
        # #print([c3_t_att.shape, roi3t.shape])[torch.Size([30, 256, 36, 36]), torch.Size([240, 256, 5, 5])]
        # #print([c4_t_att.shape, roi4t.shape])[torch.Size([30, 256, 18, 18]), torch.Size([240, 256, 3, 3])]

        #we skip the prroi_pooling for test image
        roi3t = c3_t_att
        roi4t = c4_t_att
        ...
        wh_pred = self.wh_predictor(fc34_rt_cat)
        wh_pred_01 = self.sigmoid(wh_pred).view(batch_size, 1, 2)
        return wh_pred_01
